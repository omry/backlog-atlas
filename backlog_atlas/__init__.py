...
def run_update(args: argparse.Namespace) -> int:
    ...
    if not updates_jsonl_path.exists() and updates_path.exists():
        seed_events = parse_updates_md(read_text(updates_path))
        if seed_events:
            bootstrapped = True
            if not args.dry_run:
                append_events_jsonl(updates_jsonl_path, seed_events)
    ...
    if args.dry_run and seed_events:
        recent_events = list(reversed(seed_events)) + recent_events
...
def load_workflow_template(pip_spec: str) -> str:
    return (
        ...
        .replace("__BACKLOG_ATLAS_PIP__", pip_spec)
...
def find_workflow_target(target_repo_root: Path) -> Path:
    return target_repo_root / ".github" / "workflows" / "update-backlog-atlas.yml"
...
def run_install(args: argparse.Namespace) -> int:
    ...
    pip_spec = args.pip_spec or "backlog-atlas"
    ...
    # 1. Backlog Atlas branch
    if not args.skip_branch:
        if gh_branch_exists(repo, BACKLOG_BRANCH):
            print(f"✓ {BACKLOG_BRANCH} branch already exists")
        else:
            print(f"· creating {BACKLOG_BRANCH} branch via gh API ...")
            seed = (
                f"# Detailed Open Issues: {repo}\n\n"
                f"{BEGIN_GENERATED}\n*Generated on: -*\n{END_GENERATED}\n\n"
                f"## Manual comments\n{canonical_manual_block()}\n"
            )
            gh_create_orphan_branch(repo, BACKLOG_BRANCH, "BACKLOG.md", seed)
            print(f"✓ {BACKLOG_BRANCH} branch created")
            did_anything = True
    ...
    if did_anything:
        ...
        print(f"  - trigger once: gh workflow run 'Update Backlog Atlas' --repo {repo}")
        print(
...
def run_uninstall(args: argparse.Namespace) -> int:
    ...
    if not args.yes:
        ...
        if not args.keep_workflow:
            print(
                "  - remove .github/workflows/update-backlog-atlas.yml from the working tree"
            )
        if not args.keep_branch:
            print(f"  - DELETE the {BACKLOG_BRANCH} branch on GitHub (irreversible)")
        try:
    ...
    if not args.keep_branch:
        if gh_branch_exists(repo, BACKLOG_BRANCH):
            gh_delete_branch(repo, BACKLOG_BRANCH)
            print(f"✓ deleted {BACKLOG_BRANCH} branch on {repo}")
        else:
            print(f"· no {BACKLOG_BRANCH} branch to delete")
...
def main() -> int:
    ...
    p_install = sub.add_parser(
        "install",
        help="Set up the backlog-atlas branch and GitHub Actions workflow in a repo.",
    )
    ...
    p_install.add_argument(
        ...
        help=(
            ...
            "Default: 'backlog-atlas'. Use a git URL or 'pip install ./local-path' "
    ...
    p_install.add_argument(
        "--skip-branch",
        action="store_true",
        help="Don't touch the backlog-atlas branch.",
    )
    p_install.add_argument(
    ...
    p_uninstall = sub.add_parser(
        "uninstall",
        help="Remove the workflow and (optionally) delete the backlog-atlas branch.",
    )
    ...
    p_uninstall.add_argument(
        "--keep-branch",
        action="store_true",
        help="Don't delete the backlog-atlas branch.",
    )
    p_uninstall.add_argument(
